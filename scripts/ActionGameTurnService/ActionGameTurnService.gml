...
function tryProcessGameTurn(unitApValue) {
    ...
    if (unitApValue >= gameTurn.CURRENT_TURN_AP_THRESHOLD) {
        ...
        LOG_INFO_MESSAGE("[*******GAME TURN*******] New turn started: " + string(gameTurn.GAME_TURN_INDEX) + " Threshold: " + string(gameTurn.LAST_TURN_AP_BOUNDARY));
        resolvedAction.execute();
        LOG_INFO_MESSAGE("[*******GAME TURN*******] Finished new turn actions: " + string(gameTurn.GAME_TURN_INDEX) + " Threshold: " + string(gameTurn.LAST_TURN_AP_BOUNDARY));
